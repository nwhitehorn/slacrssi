...
int bytes_to_header(uint16_t *bytes, struct header *hdr){
	...
	hdr->ackf = (bytes[0] >> 14) & 1;
	hdr->rstf = (bytes[0] >> 12) & 1;
	hdr->nulf = (bytes[0] >> 11) & 1;
	hdr->busyf = (bytes[0] >> 8) & 1;
	hdr->seq = bytes[1] >> 8;
	hdr->ack = bytes[1] & 0xFF;

	if (hdr->synf){
		hdr->vsn = bytes[2] >> 12;
		hdr->chk = (bytes[2] >> 10) & 1;
		hdr->max_outstanding_segs = bytes[2] & 0xFF;
...
int read_header(int fd, struct header *hdr, uint16_t *buffer){
	uint16_t bytes_transferred = read(fd, buffer, BUFFER_SIZE);
	for (int i = 0; i < 12;  i++)
...
main(int argc, const char **argv)
{
	...
	uint16_t ackhdr_bytes[4], ackhdr_bytes_net[4];
	header_to_bytes(&ackhdr, ackhdr_bytes);
	for (int i = 0; i < 4; i++)
		ackhdr_bytes_net[i] = htons(ackhdr_bytes[i]);
	uint8_t resp_seq;
	...
	double time_elapsed = stop.tv_sec - start.tv_sec + (double)(stop.tv_usec - start.tv_usec)/1000000;
	printf("Sent %d bytes in %.4f seconds (%.4f MB/sec)\n", total_size,  time_elapsed, total_size/time_elapsed / 1000000);
